...
public class SelectNode extends ResultSetNode
{
	...
	/* Aggregate FastTables for select and where clauses */
	FastTable	selectAggregates ;
	FastTable	whereAggregates;
	FastTable  havingAggregates;
	/**
					...
					throws StandardException
	{
		...
		selectSubquerys = (SubqueryList) getNodeFactory().getNode(
											...
											getContextManager());
		selectAggregates = new FastTable();
		...
		if (havingClause != null) {
			havingAggregates = new FastTable();
		...
		if (groupByList != null)
		{
			FastTable gbAggregateFastTable = new FastTable();

			groupByList.bindGroupByColumns(this,
										   gbAggregateFastTable);
			...
			*/
			if (SanityManager.DEBUG)
			{
				SanityManager.ASSERT(gbAggregateFastTable.size() == 0,
						"Unexpected Aggregate FastTable generated by Group By clause");
			}
		}
				...
				throws StandardException
	{
		...
		*/ 
		if (((selectAggregates != null) && (selectAggregates.size() > 0)) 
			|| (groupByList != null))
		{
			FastTable aggs = selectAggregates;
		...
		throws StandardException
	{
		...
		return newTopRSN.isOrderedOn(crs, permuteOrdering, (FastTable)null);
	}
